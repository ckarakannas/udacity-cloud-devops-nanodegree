...
This project features a high-availability web application deployed using Infrastructure as Code (IaC) using AWS CloudFormation.
...
This architecture features EC2 instances hosted in private subnets, managed by a Launch Configuration resources and an AutoScaling Group spanning across to AZs. A load balancer is also provisioned on the two public subnets configured to forward traffic to the EC2 instances and perform health checks. 

Outbound traffic from the EC2 instances is made possible by defining routing rules to forward traffic to NAT Gateways on public subnets and then to the Internet Gateway.

Finally, an IAM S3 read-only role is attached to the EC2 instances to access the demo web-app code hosted on a public S3 bucket.
...
- <!---->stack.sh => Helper Bash script for creating, updating and deploying CloudFormation stacks using AWS CLI
